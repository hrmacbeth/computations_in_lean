...
begin
  ...
  { linear_combination (w ^ 2 + z ^ 2 + y ^ 2 + x ^ 2 + 1) * H },
  { ring },
  { ring },
  { ring },
  { linear_combination (w ^ 2 + z ^ 2 + y ^ 2 + x ^ 2 + 1) * H },
...
begin
  ...
  { -- hard direction: a quaternion in the kernel is ±1
    ...
    have h₀₂ := congr_fun₂ h 0 2,
    have h₁₂ := congr_fun₂ h 1 2,
    have h₂₂ := congr_fun₂ h 2 2,
    simp only with quaternion_simps at h₀₀ h₀₁ h₀₂ h₁₂ h₂₂ { discharger := `[norm_num1] },
    have hy : y = 0,
    { linear_combination - 2⁻¹ * y * (h₀₀ + h₂₂) - 2⁻¹ * x * h₁₂ - 2⁻¹ * z * h₀₁ },
    have hz : z = 0,
    { linear_combination - 2⁻¹ * z * (H + h₀₀) + 2⁻¹ * x * h₀₂ + 2⁻¹ * y * h₀₁ },
    have hw : w = 0,
    { linear_combination - 2⁻¹ * w * (H + h₀₀) + 2⁻¹ * y * h₀₂ - 2⁻¹ * x * h₀₁ },
    have hx : x ^ 2 = 1 ^ 2,
